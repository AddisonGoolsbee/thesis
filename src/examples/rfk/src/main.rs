...
pub unsafe extern "C" fn full_draw(mut o: screen_object, mut in_place: bool) {
    ...
    if !(stdscr as *const libc::c_void).is_null() {
        ...
        if !(&mut dummy as *mut libc::c_short as *const libc::c_void).is_null() {
            dummy = (((*stdscr)._attrs
                & ((1 as libc::c_ulong) << 8 as libc::c_int).wrapping_sub(1 as libc::c_ulong)
                    << 0 as libc::c_int + 8 as libc::c_int)
                >> 8 as libc::c_int) as libc::c_int as libc::c_short;
        } else {
        };
    } else {
        -(1 as libc::c_int);
    ...
    if o.character as libc::c_int == '#' as i32 {
        new |= (1 as libc::c_ulong) << 12 as libc::c_int + 8 as libc::c_int;
    }
    if o.character as libc::c_int <= '\u{1a}' as i32 {
        new |= (1 as libc::c_ulong) << 14 as libc::c_int + 8 as libc::c_int;
    }
    if o.bold {
        new |= (1 as libc::c_ulong) << 13 as libc::c_int + 8 as libc::c_int;
    }
    if !(stdscr as *const libc::c_void).is_null() {
        (*stdscr)._attrs = new;
    } else {
